...
const staticHandlers = {
  ...
  server(options){
    ...
    if(!rbac){
      ...
      proxyOptions = Object.assign(proxyOptions, getAgentProxyOptions(options.serverConfig));
...
function getAgentProxyOptions(serverConfig) {
  const requestOptions = zluxUtil.getAgentRequestOptions(serverConfig);
...
WebApp.prototype = {
  ...
  makeProxy(urlPrefix, noAuth, overrideOptions, host, port) {
    ...
    if (!(host && port)) {
      ...
      const requestOptions = zluxUtil.getAgentRequestOptions(this.options.serverConfig, urlPrefix);
    ...
    for (const proxiedRootService of this.options.rootServices || []) {
      ...
      if (proxiedRootService.requiresAuth === false) {
        const _router = this.makeProxy(proxiedRootService.url, true,
                                       getAgentProxyOptions(this.options.serverConfig));
