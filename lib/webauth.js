...
//TODO this seems like a hack to work-around the config service being a plugin. If it were a core api,
//Checks could exist more formally
function isGetConfigRequest(req) {
  if (req.method != 'GET' && req.method != 'HEAD') {
    return false;
  } else if (req.originalUrl.startsWith('/ZLUX/plugins/org.zowe.configjs/services/data/')) {
    return true;
  }
  return false;
}
    ...
    middleware: Promise.coroutine(function*(req, res, next) {
      try {
        ...
        if (authManager.sessionTimeoutMs !== TIMEOUT_VALUE_NO_EXPIRE
            && (!timeout || timeout < Date.now())) {
          req.session.zlux = undefined;
          if (isGetConfigRequest(req)) {
            next();//delegate to config service
            return;
          } else {
