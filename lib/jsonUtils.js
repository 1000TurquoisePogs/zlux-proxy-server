...
const unmunge = [
  ...
  //ax
  0x41, 0x00, 0x4a, 0xb1, 0x9f, 0xb2, 0x6a, 0xb5, 0xbb, 0xb4, 0x9a, 0x8a, 0xb0, 0xca, 0xaf, 0xbc,
  //bx
...
function convert1047ToUnicode(buf) {
  let i = 0;
  let outBuf = Buffer.alloc(buf.length);
  let c = buf[i];
  console.log('----- c is '+c);
  while ( c !== undefined) {
    outBuf[i] = map1047ToUnicode[c];
    i++;
    c = buf[i];
  }
  return outBuf;
...
function convertMungedToUnicode(buf) {
  let i = 0;
  let outBuf = Buffer.alloc(buf.length);
  let c = buf[i];
  while ( c !== undefined) {
    outBuf[i] = unmunge[c];
//    console.log('0x'+c.toString(16)+'('+String.fromCharCode(c)
//                +')=>0x'+outBuf[i].toString(16)+'('+String.fromCharCode(outBuf[i])+')');
    i++;
    c = buf[i];
  }
  return outBuf;
...
function stripComments(fileAsString) {
  var cleanJSON = ""; 
  var pos = 0;
  var done = false;
  while (!done){
...
JSONUtils.readJSONFileWithComments = function(filename){
  ...
  try {
    ...
    let cleanJSON = stripComments(fileAsString);
    parsedJSON = JSON.parse(cleanJSON);
  } catch (e) {
    console.log('Warning: Could not read JSON normally, attempting alternative encoding');
    let fileAsString = convertMungedToUnicode(fileBuffer).toString();
    let cleanJSON = stripComments(fileAsString);
