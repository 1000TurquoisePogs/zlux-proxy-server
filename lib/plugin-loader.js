...
PluginLoader.prototype = {
  ...
  getComponentCapabilities(config) {
    return new Promise((complete, fail)=> {
      let appServerComp = {};
      let agentComp = {};
      const agentOptions = zluxUtil.getAgentRequestOptions(config);
      ...
      if (!agentOptions) {
        ...
        const clientRequest = agentOptions.protocol == 'https:' ? https : http;
        agentOptions.path = '/server/agent/environment';
        return new Promise((resolve, reject) => { /* Obtains and stores environment information from agent */
          clientRequest.get(agentOptions, (res) => {
          ...
          agentOptions.path = '/server/agent/services';
          return new Promise((resolve, reject) => { 
            clientRequest.get(agentOptions, (res) => {
              const { statusCode } = res; // TODO: Check status code for bad status
              const contentType = res.headers['content-type'];
